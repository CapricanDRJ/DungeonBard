...
// Returns the session if valid (regardless of whitelist), for checking login status
...
// Root: server-side auth gate
app.get('/', (req, res) => {
  const validSession = getValidSession(req);
  const authedSession = getAuthedSession(req);
...
<div class="card">
  ...
  // Logged in but not on whitelist — send to quest registry
  if (!authedSession) return res.redirect('/quests');

  // Logged in and whitelisted — show admin panel
  const username = authedSession.username || 'Scribe';
