        ...
        case "questcomplete":
          ...
          if (unixTime < completeTime) {
            ...
            for (const item of activeItems) {
              ...
              itemString += item.skillBonus ? `\n- <:emoji:${item.emojiId}> ${item.name} (Skill: ${skillNames[item.skill - 1]}, X${item.skillBonus})` : '';
              itemString += item.professionBonus ? `\n- <:emoji:${item.emojiId}> ${item.name} (Profession: ${professionNames[parseInt(item.professionId) - 1]} +${item.professionBonus})` : '';
            }
