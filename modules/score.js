...
module.exports = {
    ...
    executeCommand: async (interaction) => {
        ...
        try {
            ...
            // Find calling user and their rank
            const callingUser = allUsers.find(u => u.isCallingUser === 1);
            ...
            const userRank = callingUser.callingUserRank;
            const avatars = dbQuery.getAvatarBlobs.all(guildId, guildId);

            let displayUsers;
            let callingUserIndex;

            // If user is in top 20, show top 20 with user at their position
            if (userRank <= 20) {
                displayUsers = allUsers;
                callingUserIndex = userRank - 1;
            } else {
                // User is outside top 20, center them in the middle (position 10)
                displayUsers = allUsers.slice(0, 10);
                displayUsers.push(callingUser);
                callingUserIndex = 10;
            }

            // Generate scoreboard image
            const imageBuffer = await generateScoreboardImage(displayUsers, avatars, callingUserIndex, userRank);
