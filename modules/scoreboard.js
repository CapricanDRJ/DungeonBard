    ...
    try {
        ...
        for (const user of users) {
            ...
            if(rank === userLength+1) {
                ...
                const bannerTop = y - gainOffset - Math.floor((h1Height - ROW_HEIGHT) / 2); // Center the ribbon on the row, with a slight upward nudge
                compositeLayers.push({
                    input: hrBuffer,
                    top: bannerTop,
                    left: bannerLeft
                });
                svgContent += `<text x="${bgWidth / 2}" y="${y + 3 + Math.floor(ROW_HEIGHT / 2) - gainOffset}" text-anchor="middle" class="h2">Folios of Renown</text>`;
            };
